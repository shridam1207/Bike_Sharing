...
var introAnimation, //timeline for intro animation
	...
	meterHappiness = 0, //placeholder for happiness meter start
	meterLoans = 0, //placeholder for loan meter start
	selectedCollegeCost,
	meterConnections = 0, //placeholder for connections meter start
...
// $c1 = document.getElementById("b1")
// $c1.onclick() = function () { window.scrollTo(0, 200); };
...
function my_card_back(e , elm){
	$var = elm.children[1].innerHTML;
	$var = $var.split("")
	$var = parseInt($var[1] + $var[2]);
	shop_money -= $var;
...
//  MAKE CASH MEMORIES FUNCTION HERE

// function my_cash_func(x){
// 	// console.log(x);
// 	if ( x == 1){
// 		var $var = document.getElementsByClassName("meter-happiness");
// 		// var hapid = document.getElementById("happid");
// 		var $hap_data =  parseInt($var[0].attributes[1].value);
// 		$hap_data -= 50
// 		$var[0].attributes[1].value = $hap_data
// 		$("#haid")[0].lastChild.previousSibling.childNodes[0].data = "-"
// 		$("#haid").css({"opacity" : "1"})
// 		setTimeout(function () {
// 			$("#haid").css({"opacity" : "0"});
// 		}, 2000);
// 		// console.log($hap_data)
// 		$('#happid').css({"transition" : "width 2s" , "width" : $hap_data/10+ "%"});
// 		calculateLoanAndApplyChange(loan + 100);
// 		loan = loan + 100;
// 	}
// 	else if(x ==2){
// 		var $var = document.getElementsByClassName("meter-happiness");
...
function books(x){
	...
	if ( x == 0){
		...
		// Money code
		calculateLoanAndApplyChange(loan + 8000);
		loan = loan + 8000;
		$("#loid")[0].children[0].innerHTML = "+" + 8000; 
		$("#loid").css({"opacity" : "1"})
		setTimeout(function () {
			$("#loid").css({"opacity" : "0"});
		}, 2000);
		...
		// Loan =================
		calculateLoanAndApplyChange(loan+ 0);
		loan = loan+0
		$("#loid")[0].children[0].innerHTML = "+" + 0; 
		$("#loid").css({"opacity" : "1"})
		setTimeout(function () {
			$("#loid").css({"opacity" : "0"});
		}, 2000);
		...
		// Loan =======================
		calculateLoanAndApplyChange(loan + 1000);
		loan = loan + 1000;
		$("#loid")[0].children[0].innerHTML = "+" + 1000; 
		$("#loid").css({"opacity" : "1"})
		setTimeout(function () {
			$("#loid").css({"opacity" : "0"});
		}, 2000);
...
function laptop(x){
	...
	if ( x == 0){
		...
		// Money code
		calculateLoanAndApplyChange(loan + 60000);
		loan = loan + 60000;
		...
		// Loan =================
		calculateLoanAndApplyChange(loan+30000);
		loan+=30000;
	}
	else if(x ==2){
		// Focus code =====================
